...
# Audio is scoped to the Numbers deck only. Clips live here (generated by
# generate_number_audio.py); NUMBERS_MEDIA_FILES accumulates the mp3/wav paths actually
# referenced so they can be bundled into Numbers.apkg — and only that package.
NUMBERS_MEDIA_DIR = os.path.join(os.path.dirname(__file__), "../anki/numbers/media")
NUMBERS_MEDIA_FILES: set[str] = set()
...
def process_numbers_card(card, deck):
    direction = card.get("direction")
    ...
    audio = find_audio(NUMBERS_MEDIA_DIR, spanish)
    spanish_field = spanish
    if audio:
        spanish_field = f"{spanish} [sound:{audio}]"
        NUMBERS_MEDIA_FILES.add(os.path.join(NUMBERS_MEDIA_DIR, audio))
...
def process_json_files():
    ...
    for deck_name, package_data in packages.items():
        ...
        # Only the Numbers deck carries audio; every other package ships without media_files.
        media_files = (
            sorted(NUMBERS_MEDIA_FILES)
            if package_data["config"].get("model_type") == "numbers"
            else []
        )
