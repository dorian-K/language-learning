...
5c. `generate_sentence_audio.py` + `tts.py` — **same offline TTS, for the example sentences** (`example_sentence_es`) on the **conjugation** cards (the vocab deck voices the headword instead — see 5d). Collects every unique sentence across `anki/irregular_verbs/`, `anki/regular_verbs/` and synthesizes one clip per sentence into a single shared pool `anki/sentence_audio/` (idempotent, content-hash-named, same backends/env as 5b). **The spoken text comes from `spoken_sentence(card)` (`spoken_sentence.py`), not the raw field** — forward conjugation cards blank the verb as `[infinitive]` in `example_sentence_es`, so it substitutes `conjugated_form` back in (`No [volver]…` → `No volváis…`) so the audio speaks the real conjugated form; reverse cards have no bracket, so it's a no-op. **Standalone** (imports only `tts` + `spoken_sentence`, both pure/no genanki) so it runs on the cluster with just the `tts` extra. Cluster wrapper: `slurm/generate_sentence_audio.slurm`. Unlike numbers, its **input JSON is gitignored** — rsync `anki/` up to the cluster first, run there, rsync `anki/sentence_audio/` back.
5d. `generate_word_audio.py` + `tts.py` — **same offline TTS, but for the Spanish headwords** on the Refold ES1K **vocab** deck (the vocab deck voices the *word*, not the sentence). Collects every unique headword across `anki/<VOCAB_SOURCE>/` into the shared pool `anki/word_audio/` (idempotent, content-hash-named, same backends/env as 5b). **The spoken text comes from `spoken_word(card)` (`spoken_word.py`)** — the `cue_spanish` for `spanish_to_target` cards and the primary `target_es[0]` for `target_to_spanish`, so both directions of the same item share one clip. **Standalone** (imports only `tts` + `spoken_word`, both pure/no genanki). Cluster wrapper: `slurm/generate_word_audio.slurm`; same gitignored-input rsync round-trip, but sync `anki/word_audio/` back. Tested in `tests/test_spoken_word.py` + `tests/test_vocab_word_audio.py`.
6. `make_anki_deck.py` — reads `anki/<VOCAB_SOURCE>/` (vocab) + `anki/irregular_verbs/` + `anki/regular_verbs/` (conjugations) + `anki/numbers/`, builds level-organised decks (`Lt::levelA1` … `Lt::levelB2`, `Lt::Conjugations::High Priority` + `Lt::Conjugations::Low Priority`, `Lt::Regular Conjugations`, `Lt::Numbers`), exports `.apkg`. Three model types dispatched by `model_type` (`vocab`/`conjugation`/`numbers`); a new deck kind needs a new model + a `process_*_card` + an `INPUT_CONFIGS` entry + a dedup-key branch. **Deck organisation** is driven by each config's `deck_naming`: `level` (vocab → per-CEFR subdecks), `flat` (one deck named by `deck_name`), or `priority` (irregular verbs only → split into `::High Priority` / `::Low Priority` subdecks by `conjugation_tier(card)`, which tests the card's `infinitive` against the curated `HIGH_PRIORITY_IRREGULARS` set — organisation-only, every card is still built with full audio). **Audio** (Duolingo-style autoplay): the shared `sound_suffix(text, media_dir, media)` helper appends `[sound:…]` to the field carrying the Spanish text only when a clip exists — number words from `anki/numbers/media/` (Spanish side of numbers cards), vocab headwords from `anki/word_audio/` (keyed by `spoken_word(card)`), conjugation example sentences from `anki/sentence_audio/` (keyed by `spoken_sentence(card)` so forward-conjugation clips match the conjugated, not `[infinitive]`, text). Placement: **vocab** speaks the *word* — `spanish_to_target` → front word (`Front_Word`); `target_to_spanish` → the answer word (`Back_Word`, autoplays on flip); **reverse** conjugation → front sentence (already shows the conjugated form); **forward** conjugation → the **answer word** (`Back_Word`), so the corrected full sentence autoplays on flip rather than giving the answer away up front. Each `process_*_card` takes a per-package `media` set; the packaging step bundles exactly the clips that package referenced (`media_files=sorted(package_data["media"])`), so a deck with no audio present builds silently, unchanged.
...
7b. `sync_anki_edits.py` — **preferred reverse-sync for the Refold ES1K vocab deck** (supersedes `sync_anki_changes.py` for vocab; conjugation decks not yet supported). A `diff` → reviewable `patch.json` → `apply` workflow: `diff` parses the exported `.apkg` and isolates *your* hand-edits from source drift two ways — (a) **modification time**: `notes.mod` clusters the bulk import on one day, so only notes touched after it are candidates (override with `--since` / `--all` / `--baseline`); (b) **HTML-normalized content diff** (`canon()` folds `class='x'`↔`"x"`, `<br>`↔`\n`, strips `[sound:]`). The parser round-trips exactly against `make_anki_deck.process_vocab_card` (asserted in tests), and cards are matched by the deterministic `source_guid` (must stay in lockstep with `make_anki_deck`'s GUID key). `--skip-multiline` drops entries whose new value is newline-joined sentences (a build-time duplicate-merge artifact, not a hand edit). `apply` backs up the source dir and only overwrites fields whose current value still matches the patch's `old` (conflicts skipped unless `--force`). For clean future syncs, run `snapshot` right after each build+import and later `diff --baseline` against it. Tested in `tests/test_sync_anki_edits.py`.
